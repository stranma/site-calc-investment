...
class TestInvestmentClientInitialization:
    ...
    def test_client_initialization(self):
        ...
        client = InvestmentClient(base_url="https://api.example.com", api_key="inv_test_key")
    ...
    def test_client_requires_inv_prefix(self):
        ...
        # Valid
        InvestmentClient(base_url="https://api.example.com", api_key="inv_valid_key")

        # Invalid: wrong prefix
        with pytest.raises(ValueError, match="must start with 'inv_'"):
            InvestmentClient(base_url="https://api.example.com", api_key="op_wrong_prefix")

        # Invalid: no prefix
        with pytest.raises(ValueError, match="must start with 'inv_'"):
            InvestmentClient(base_url="https://api.example.com", api_key="no_prefix")
...
class TestCreatePlanningJob:
    ...
    def test_create_planning_job_success(self, mock_request, simple_site, prague_tz, mock_job_response):
        ...
        timespan = TimeSpanInvestment(
            start=datetime(2025, 1, 1, 0, 0, 0, tzinfo=prague_tz), intervals=8760, resolution=Resolution.HOUR_1
        )
    ...
    def test_create_planning_job_forbidden_feature(self, mock_request):
        ...
        mock_response.json.return_value = {
            "error": {"code": "forbidden_feature", "message": "Ancillary services not available for investment clients"}
        }
        mock_request.return_value = mock_response
...
class TestGetJobStatus:
    ...
    def test_get_job_status_not_found(self, mock_request):
        ...
        mock_response.json.return_value = {"error": {"code": "job_not_found", "message": "Job not found"}}
        mock_request.return_value = mock_response
...
class TestCancelAllJobs:
    ...
    def test_cancel_all_jobs_success(self, mock_request):
        ...
        mock_response.json.return_value = {
            "cancelled_count": 3,
            "cancelled_jobs": ["job_id_1", "job_id_2", "job_id_3"],
...
class TestWaitForCompletion:
    ...
    @patch("httpx.Client.request")
    @patch("time.sleep")
    def test_wait_for_completion_failed(self, mock_sleep, mock_request, mock_job_failed_response):
    ...
    @patch("time.time")
    @patch("time.sleep")
    def test_wait_for_completion_timeout(self, mock_sleep, mock_time, mock_request, mock_job_running_response):
...
class TestRetryLogic:
    ...
    def test_no_retry_on_client_error(self, mock_sleep, mock_request):
        ...
        mock_response = Mock()
        mock_response.status_code = 400
        mock_response.json.return_value = {"error": {"code": "validation_error", "message": "Invalid"}}
        mock_request.return_value = mock_response
